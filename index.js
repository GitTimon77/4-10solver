...
// NEW: default target result is 10
let ResultSearch = 10

// NEW: connect input field (if present)
const resultSearchInput = document.querySelector("#resultSearch")
if (resultSearchInput) {
    resultSearchInput.value = ResultSearch
    resultSearchInput.addEventListener("input", () => {
        const v = Number(resultSearchInput.value)
        ResultSearch = Number.isFinite(v) ? v : 0
        showSolution()
    })
}
            ...
            for (let j = 0; j < oprs.length; j++) {
                ...
                for (let k = 0; k < oprs.length; k++) {
                    ...
                    for (let m = 0; m < 6; m++) {
                        ...
                        if (mathAns == ResultSearch) return print

                        if (Math.abs(mathAns - ResultSearch) < Math.abs(bestGuessMath - ResultSearch)) {
...
// initial render (so title matches current default ResultSearch)
showSolution()
